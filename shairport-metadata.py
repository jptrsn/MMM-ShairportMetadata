...
if __name__ == "__main__":
    ...
    while True:
        ...
        if typ == "core":
            if code == "asal":
                metadata['Album Name'] = data.decode('utf-8', errors='ignore')
            elif code == "asar":
                metadata['Artist'] = data.decode('utf-8', errors='ignore')
            elif code == "minm":
                metadata['Title'] = data.decode('utf-8', errors='ignore')
        if typ == "ssnc" and code == "snam":
            metadata['snam'] = data.decode('utf-8', errors='ignore')
        if typ == "ssnc" and code == "prgr":
            metadata['prgr'] = data.decode('utf-8', errors='ignore')
        ...
        if typ == "ssnc" and code == "PICT":
            if len(data) == 0:
                ...
                print(json.dumps({"image": f"data:{mime};base64,{base64.b64encode(data).decode('utf-8')}"}))
            sys.stdout.flush()
